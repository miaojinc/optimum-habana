...
def reshape_for_broadcast_ri(freqs_cos: torch.Tensor, freqs_sin: torch.Tensor, x: torch.Tensor):
    ndim = x.ndim
    assert freqs_cos.shape == (x.shape[-3], x.shape[-2]), \
        f"freqs_cos shape {freqs_cos.shape} != {(x.shape[-3], x.shape[-2])}"
    assert freqs_sin.shape == freqs_cos.shape, "cos/sin shape is diff"
    shape = [1] * (ndim - 2) + [x.shape[-3], x.shape[-2]]
    cos = freqs_cos.view(*shape)
    sin = freqs_sin.view(*shape)
    return cos, sin

def apply_rotary_enc_ri(
    xq: torch.Tensor,
    xk: torch.Tensor,
    ...
    cos, sin = reshape_for_broadcast_ri(freqs_cos.to(xq_pairs.dtype),
            freqs_sin.to(xq_pairs.dtype),
            xq_pairs)
    q_even = xq_pairs[..., 0]
    q_odd  = xq_pairs[..., 1]
    q_out_even = q_even * cos - q_odd * sin
    q_out_odd  = q_even * sin + q_odd * cos
    xq_out = torch.stack((q_out_even, q_out_odd), dim=-1).reshape(*xq.shape)
    if xk.shape[-2] == 0:
        return xq_out.type_as(xq).to(xq.device), xk
    ...
    xk_out = torch.stack((k_out_even, k_out_odd), dim=-1).reshape(*xk.shape)
    return xq_out.type_as(xq).to(xq.device), xk_out.type_as(xk).to(xk.device)
